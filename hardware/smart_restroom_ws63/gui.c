...
void GUI_ShowFont16(uint8_t x,uint8_t y,uint8_t *s,uint8_t mode)
{
	...
	int find=0;
  	...
  	for(i=0;i<num;i++)
	{
		if( (cfont16[i].Index[0]==*s)
		&&(cfont16[i].Index[1]==*(s+1))
		&&(cfont16[i].Index[2]==*(s+2)) ){
			find = 1;
			for(j=0;j<32;j++)
			{
				if(mode){
 ...
 for(i=0;i<height;i++)
 {
		for(j=0;j<(width+7)/8;j++)
		{
		    if(mode)
				...
				for(k=0;k<8;k++)
				{
					OLED_Set_Pixel(x+j*8+k, y+i, (tmp>>k)&0x01 ? 1 : 0);
...
void GUI_FillData(uint8_t x,uint8_t y,uint8_t width, uint8_t height, uint8_t data[],uint8_t dataLen, uint8_t mode)
{
	GUI_DrawBMP(x,y,width, height, data,  mode);
}

void ssd1306_DrawRegion(uint8_t x, uint8_t y, uint8_t w, uint8_t h, const uint8_t* data, uint32_t size, uint32_t stride)
{
    if (x + w > WIDTH || y + h > HEIGHT || w * h == 0) {
    ...
    h = (h <= HEIGHT ? h : HEIGHT);
	//dir 0:行列式    1:列行式  
    stride = (stride == 0 ? h : stride);

	uint8_t cols = size * 8 / stride;
	int lastj=0;
	for (uint8_t i = 0; i < cols; i++) {		
		
		for (uint8_t j=0; j < h; j++) {
			
			uint32_t idx = 0 + (lastj / 8);
			uint8_t byte = idx < size ? data[idx] : 0;
			
			uint8_t bit  = byte & (0x01 << (lastj % 8));
			
			OLED_Set_Pixel(x + i, y + j, bit ? 1 : 0);
			lastj++;
		}
	}
}
